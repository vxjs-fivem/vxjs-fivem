import { ApplicationBuilder, IBinder, NetEvent, LocalEvent, Controller } from '@vxjs-fivem/core/src';
    ...
    const binder = new (class NetEventBinder implements IBinder {
      public bind(controller: unknown): void {
        ...
        expect(metadata[0].value)
          .toStrictEqual({ name: 'netEvent' });
        expect(metadata[0].method)
          .toBe('netEvent');
        expect(metadata[0].kind)
          .toBe('method');
      }
    })();
    ...
    const binder = new (class LocalEventBinder implements IBinder {
      public bind(controller: unknown): void {
        ...
        expect(metadata)
          .toHaveLength(2);
        expect(metadata[0].value)
          .toStrictEqual({ name: 'localEvent1' });
        expect(metadata[0].method)
          .toBe('localEventMethod1');
        expect(metadata[0].kind)
          .toBe('method');

        expect(metadata[1].value)
          .toStrictEqual({ name: 'localEvent2' });
        expect(metadata[1].method)
          .toBe('localEventMethod2');
        expect(metadata[1].kind)
          .toBe('method');
      }
    })();
